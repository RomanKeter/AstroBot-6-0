...
Планеты: формула Мееуса (Jean Meeus "Astronomical Algorithms").
Луна: точность ~0.3°, знак верный в 98% случаев.
Натальная карта: 10 планет + асцендент + 12 домов (Equal House).
"""
...
# Описания 12 домов — западная астрология
HOUSE_MEANINGS = {
...
# Управители знаков (западная астрология)
SIGN_RULERS = {
...
def _moon_longitude_meeus(year: int, month: int, day: int, hour: float = 12.0) -> float:
    """
    Точная долгота Луны по Мееусу (глава 47).
    Погрешность < 0.3°. Знак Луны верный в ~98% случаев.
    """
    ...
    sigma_l = (
        ...
        -    6766 * math.sin(2*D_r + M_r)
        -    5163 * math.sin(D_r - Mp_r)
        +    4987 * math.sin(D_r + M_r)
        +    4036 * math.sin(2*D_r - M_r + Mp_r)
        +    3994 * math.sin(2*D_r + 2*Mp_r)
        +    3861 * math.sin(4*D_r)
...
def _planet_mean_longitude(year: int, month: int, day: int, hour: float = 12.0) -> Dict[str, float]:
    """
    Средние долготы планет (Мееус, упрощённые).
    Точность: Меркурий/Венера ±2°, Марс ±5°, внешние ±10–20°.
    Для точной карты нужен kerykeion — но знаки будут правильными.
    """
    jd = _julian_day(year, month, day, hour)
    d  = jd - 2451545.0
    T  = d / 36525.0
...
def _ascendant_longitude(year: int, month: int, day: int,
    ...
    """Долгота асцендента через Local Sidereal Time."""
...
def _planet_in_house(planet_lon: float, cusps: Dict[int, float]) -> int:
    """Номер дома для данной долготы (Equal House)."""
...
def calculate_natal_chart(
    ...
    Порядок попыток:
      1. kerykeion (точные эфемериды) — если установлен
      2. Формулы Мееуса (погрешность 0.3–20° в зависимости от планеты)
    """
    ...
    try:
        ...
        for p in ['sun', 'moon', 'mercury', 'venus', 'mars',
            ...
            if obj:
                planets[p.capitalize()] = {
                    ...
                    'sign_degree': round(obj.get('position', obj['abs_pos'] % 30), 1),
                    'house':      obj.get('house', ''),
        ...
        # Пробуем получить дома из kerykeion
        ...
        logger.info(f"Kerykeion недоступен ({e}), использую формулы Мееуса")
...
def format_full_chart_text(chart: Dict) -> str:
    ...
    lines += ["", "🏠 <b>12 домов (Equal House):</b>"]
    for i in range(1, 13):
...
def build_houses_prompt(chart: Dict, user_name: str = "пользователь") -> str:
    """Промпт для AI: полная интерпретация 12 домов."""
    lines = [
    ...
    for key in ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars',
        ...
        p = chart['planets'].get(key)
        if p:
            lines.append(f"{p['name']}: {p['sign']}, Дом {p.get('house', '?')}")
...
def build_partner_prompt(chart: Dict, name: str) -> str:
    """Промпт для натальной карты партнёра."""
    lines = [
...
def build_compatibility_prompt(chart1: Dict, chart2: Dict,
    ...
    """Промпт для AI-интерпретации совместимости."""
    lines = [
